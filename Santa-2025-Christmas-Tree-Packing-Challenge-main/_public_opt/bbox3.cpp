...
struct Cfg {
    ...
    tuple<double, double, double, double> getBBox() const {
        ...
        return std::make_tuple(gx0, gy0, gx1, gy1);
    }

    vector<int> findCornerTrees() const {
        double gx0, gy0, gx1, gy1;
        std::tie(gx0, gy0, gx1, gy1) = getBBox();
        double eps = 0.01;
        vector<int> corners;
        for (int i = 0; i < n; i++) {
    ...
    for (int it = 0; it < iter; it++) {
        ...
        if (moveType < 13) { 
            ...
            if (moveType < 4) { 
                ...
                double gx0, gy0, gx1, gy1;
                std::tie(gx0, gy0, gx1, gy1) = cur.getBBox();
                ...
                for (int k = 0; k < c.n; k++) {
                    Complex c_k = cur.c[k];
                    double d0 = abs(c_k.real() - gx0);
                    double d1 = abs(c_k.real() - gx1);
                    double d2 = abs(c_k.imag() - gy0);
                    double d3 = abs(c_k.imag() - gy1);
                    double min_dist_to_edge = std::min(std::min(d0, d1), std::min(d2, d3));
...
map<int, Cfg> loadCSV(const string& fn) {
    ...
    while (getline(f, ln)) {
        ...
        data[n].push_back(std::make_tuple(idx, stod(xs), stod(ys), stod(ds)));
    }
    for (auto& kv : data) {
        int n = kv.first;
        vector<tuple<int, double, double, double>>& v = kv.second;
        Cfg c;
        c.n = n;
        for (auto& row : v) {
            int i = 0;
